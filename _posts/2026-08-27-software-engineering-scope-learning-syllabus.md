...
**不做会怎样**
- 埋头闷做两周，上线才发现做出来的根本不是用户想要的，整个迭代推倒重来。
- 需求全靠口头传达、没白纸黑字，上线后各执一词："这不是我当初说的""你根本没提过"，扯皮到怀疑人生。
- 谁都能随口加需求、没人管边界，功能越堆越多，deadline 一拖再拖，永远做不完。
...
**不做会怎样**
- 改一个小按钮，牵一发动全身，东边补好西边又崩，谁也说不清到底影响哪些地方。
- 新人接手代码像看天书，光是搞懂"这一坨是干嘛的"就得耗上大半个月。
- 模块之间黏成一团"大泥球"，加个功能得先绕开一堆坑，越往后加得越慢、越不敢动。
...
**不做会怎样**
- 三个月后回头看自己写的代码，一脸茫然："这段是我写的？当时到底在想什么？"
- 变量名 a、b、tmp2 满天飞，同一段逻辑复制粘贴了五处，改的时候总漏掉一两处。
- 代码盘根错节，一改就冒新 bug，想重构又不敢动，只能一直往上打补丁。
...
**不做会怎样**
- 每次上线都心惊胆战，靠人肉手点一遍关键路径，点完还是不放心。
- 改了 A 功能顺手弄坏了 B 功能，没人发现，直到用户报障才知道又出事了。
- 回归全靠人工重复点，越迭代越慢，最后大家宁可不改也不敢碰老代码。
...
**不做会怎样**
- 部署靠一份"祖传手册"手工敲命令，一次上线折腾两小时，还总在某一步漏了什么。
- "在我机器上明明是好的"成了口头禅，环境不一致的问题查到天亮也复现不出来。
- 一出事想回滚，却发现根本没有干净的回滚办法，只能硬着头皮往前修。
...
**不做会怎样**
- 线上早就挂了，自己毫不知情，直到用户投诉、老板转发截图才后知后觉。
- 出了问题只能瞎猜，加几行日志、重新发一版再看，来回折腾好几轮才摸到线索。
- 半夜被电话叫醒，对着一堆没有指标、没有链路的黑盒排查一整宿，还是定位不到根因。
...
**不做会怎样**
- 每个人看起来都忙得脚不沾地，可交付日期就是一拖再拖，谁也说不清卡在哪。
- 需求和进度全装在某个人脑子里，这人一请假或离职，整个项目立刻抓瞎。
- 排期全靠拍脑袋估，说三天做完实际做两周，次次跳票，慢慢没人再信任何时间承诺。
...
**不做会怎样**
- 技术债像滚雪球一样越滚越大，直到某天谁都不敢动、只能推倒重写。
- 团队到底快不快、稳不稳，全凭感觉说不清，向上汇报只能"我觉得还行"。
- 一头扎进投入产出极低的事，忙了半天做的都是没人用、不值得做的功能。
...
**不做会怎样**
- 关键知识只锁在个别人脑子里，这人一请假，一个小问题全组都搞不定。
- 同一个坑全组轮着踩，每个人都重新掉一遍，谁也没把经验沉淀下来。
- 代码评审要么走过场随手点"同意"，要么变成互相较劲的战场，都起不到把关的作用。
...
**不做会怎样**
- 密钥、口令图省事直接硬编码进代码，一旦泄露就是上新闻、连夜改配置的大事故。
- 平时完全不管安全，上线前临时拉安全扫描，一下冒出几十个漏洞，只能通宵救火硬堵。
- 一次数据泄露或越权事故，赔偿、罚款加上信任崩塌，可能把全年的利润都搭进去。
